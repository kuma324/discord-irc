...
class Bot {
  constructor(options) {
    ...
    this.discordStatusNotices = options.discordStatusNotices;
    ...
    this.discordChannels = Object.keys(this.channelMapping).map(v=>v.substr(1));
    this.ircChannels = Object.keys(this.invertedMapping);
  }
  ...
  attachListeners() {
    ...
    this.discord.on('presenceUpdate', (oldMember, newMember) => {
      if(!this.discordStatusNotices) return;

      // TODO compare the discord server name with somewhere
      this.discord.guilds.cache.forEach((guild,key)=>{
        if(guild.channels.cache.some(c=> this.discordChannels.includes(c.name) )) {
          var text = this.convertDiscordUserPresenseToIrcText(guild, newMember);
          this.sendNoticeToIRC(newMember.user, guild, 'Discord: ' +  text);
        }
      })
    ...
    this.ircClient.on('join', (channelName, nick) => {
      logger.debug('Received join 2:', channelName, nick);
      if (!this.discordStatusNotices) return;
      if (!this.ircChannels.includes(channelName)) return;
      ...
      this.discord.guilds.cache.forEach((guild,key)=>{
        if(guild.channels.cache.some(c=> this.discordChannels.includes(c.name) )) {
          ...
          guild.members.cache.forEach((member, key) =>{
            if(member.user.id === this.discord.user.id) return;

            if(member.presence.status !== 'offline') {
              var text = this.convertDiscordUserPresenseToIrcText(guild, member.presence, member.user);
              presensesToIrc.push(text);
            }
          })

          this.sendNoticeToIRC(null, guild, 'Discord: ' + presensesToIrc.join(', '));
        }
      });
    });
  ...
  sendNoticeToIRC(author, guild, text) {
    // Ignore messages sent by the bot itself:
    if(author) {
      if (author.id === this.discord.user.id ||
          Object.keys(this.webhooks).some(channel => this.webhooks[channel].id === author.id)
    ...
    guild.channels.cache.forEach(_discordChannel =>{
        var discordChannel = '#' + _discordChannel.name;
        var ircChannel = this.channelMapping[_discordChannel.id] || this.channelMapping[discordChannel];
        if(ircChannel) {
          logger.debug(`Sending notice to IRC(${ircChannel}) from Discord(${discordChannel}) - ${text}`)
  ...
  convertDiscordUserPresenseToIrcText(guild, presense, optUser = null) {

    var user = optUser || presense.user;

    var acts = presense.activities.map(st => {
      var r = st.name;
      if(st.state) {
        if(r) {
          r += ' - ';
    ...
    var name = Bot.getDiscordNicknameOnServer(user, guild)

    return `${name} is ${presense.status}${actsPart}`;
  }
